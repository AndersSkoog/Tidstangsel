const redis         = require('redis'); // used for session storage
const connect_redis = require('connect-redis'); //used for sesssion storage
const express       = require('express');
const session       = require('express-session');
const rateLimit     = require('express-rate-limit'); //rate limiters can be applied for security counter meassures
const crypto        = require('crypto'); 
const uuid          = require('uuid'); // used to generate a unique nonce which must match the one in the session in order to activate the session, added to prevent bots and malicious activity 
...
if(process.env.NODE_ENV === "production"){
    app.set('trust-proxy',true) //Should work when run behind a mangaged proxy like Google Cloud Run, Railway, Herouku
}
...
//Ensure the browser does not cache files generated by ffmpeg
...
//middleware to check if a request has a valid session
...
function handlePerimEnter(req,res){
	...
	//start stream if no one is curently within the perimiter, and then store its nonce in the session cookike 
        tidstangsel_audio_streamer.start_stream();
        sessions_within_perim.add(req.sessionID);
...
function handlePerimExit(req, res){
    if(sessions_within_perim.size === 1){
        tidstangsel_audio_streamer.stop_stream();
        sessions_within_perim.delete(req.sessionID);
        res.status(200).json({message:"ok"});
    }
    else {
        sessions_within_perim.add(req.sessionID);
